...
| UI                | Plain CSS + lucide-react                | 当前 MVP 保持轻量、少依赖的一致操作界面       |
...
| DB Schema         | Drizzle table schema + repository layer | 用 Drizzle 描述表结构，仓储层封装 SQLite 访问 |
| Search            | 后续 SQLite FTS5                        | 经历库和岗位文本检索                         |
| Vector Search     | 后续 sqlite-vec 或独立向量库            | 语义匹配经历和 JD                            |
...
- Drizzle table schema + Zod 能让数据结构、接口输入和 AI 输出更可控；MVP 仓储层直接封装 SQLite，避免过早引入复杂查询抽象。
